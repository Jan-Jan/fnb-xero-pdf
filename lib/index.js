...
const reTransaction = /([0-9]{2}\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec))\s+([\#\*\w\s(?!\s)]+)\s\s+([\w\*\#\s]*)\s\s+([0-9]*[\s\,]{0,1}[0-9]{1,3}\.[0-9]{2}\s{0,1}[Cr]{0,2})+/
const reAccountNumber = /(account\snumber|account|credit\scard)\s+([0-9\s]+)/i
...
const filePaths = process.argv[2] && process.argv[2].slice(-3) === 'pdf'
  ? [process.argv[2]]
  : fs.readdirSync(process.argv[2] || './')
      .filter(file => file.slice(-3) === 'pdf')
    ...
    const transactions = lines
      .map(line => {
        ...
        if (match) {
          ...
          const tAmount = match[5].slice(-2) === `Cr`
            ? `${match[5].match(/[0-9]+[0-9\.\s\,]+[0-9]{2}/)[0].replace(/[\s\,]/g, '')}`
            : `-${match[5].replace(/[\s\,]/g, '')}`
